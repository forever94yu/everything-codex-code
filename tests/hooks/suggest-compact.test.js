...
function runTests() {
  ...
  if (test('suggests at threshold + 25 interval', () => {
    ...
    // threshold=3, so we need count=28 鈫?25 calls past threshold
    // Write 27 to the counter file, next run will be 28 = 3 + 25
    fs.writeFileSync(counterFile, '27');
    const result = runCompact({ CODEX_SESSION_ID: testSession, COMPACT_THRESHOLD: '3' });
    // count=28, threshold=3, 28-3=25, 25 % 25 === 0 鈫?should suggest
    assert.ok(
  ...
  if (test('resets counter on corrupted file content', () => {
    ...
    // Corrupted file 鈫?parsed is NaN 鈫?falls back to count=1
    const count = parseInt(fs.readFileSync(counterFile, 'utf8').trim(), 10);
  ...
  if (test('handles empty counter file', () => {
    ...
    const result = runCompact({ CODEX_SESSION_ID: testSession });
    assert.strictEqual(result.code, 0);
    // Empty file 鈫?bytesRead=0 鈫?count starts at 1
    const count = parseInt(fs.readFileSync(counterFile, 'utf8').trim(), 10);
  ...
  // 鈹€鈹€ Round 29: threshold boundary values 鈹€鈹€
  ...
  if (test('rejects COMPACT_THRESHOLD=0 (falls back to 50)', () => {
    ...
    // 0 is invalid (must be > 0), falls back to 50, count becomes 50 鈫?should suggest
    assert.ok(
      result.stderr.includes('50 tool calls reached'),
  ...
  if (test('accepts COMPACT_THRESHOLD=10000 (boundary max)', () => {
    ...
    // count becomes 10000, threshold=10000 鈫?should suggest
    assert.ok(
  ...
  if (test('rejects COMPACT_THRESHOLD=10001 (falls back to 50)', () => {
    ...
    // 10001 > 10000, invalid, falls back to 50, count becomes 50 鈫?should suggest
    assert.ok(
      result.stderr.includes('50 tool calls reached'),
  ...
  if (test('rejects float COMPACT_THRESHOLD (e.g. 3.5)', () => {
    ...
    // count becomes 50, threshold=3, 50-3=47, 47%25鈮? and 50鈮? 鈫?no suggestion
    assert.strictEqual(result.code, 0);
  ...
  // 鈹€鈹€ Round 64: default session ID fallback 鈹€鈹€
  ...
  if (test('uses "default" session ID when CODEX_SESSION_ID is empty', () => {
    ...
    try {
      // Pass empty CODEX_SESSION_ID 鈥?falsy, so script uses 'default'
