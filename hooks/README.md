...
|------|---------|----------|-----------|
| **Dev server blocker** | `Bash` | Blocks `npm run dev` etc. outside tmux to ensure log access | 2 (blocks) |
| **Tmux reminder** | `Bash` | Suggests tmux for long-running commands (npm test, cargo build, docker) | 0 (warns) |
| **Git push reminder** | `Bash` | Reminds to review changes before `git push` | 0 (warns) |
| **Doc file warning** | `Write` | Warns on non-standard `.md`/`.txt` files (allows README, AGENTS, CONTRIBUTING, CHANGELOG, LICENSE, SKILL, docs/, skills/, .codex/plans/) | 0 (warns) |
...
| **Formatter hook** | `Edit` | Auto-formats JS/TS files after edits (auto-detects Biome or Prettier) |
...
Remove or comment out the hook entry in `hooks.json`. If installed globally, override in your `~/.codex/settings.json`:
